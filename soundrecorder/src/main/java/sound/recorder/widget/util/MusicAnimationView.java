...
public class MusicAnimationView extends View {
    ...
    public MusicAnimationView(Context context) {
        this(context, null);
    }

    public MusicAnimationView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MusicAnimationView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttrs(context, attrs, defStyleAttr);
        initRandom();
    }
    public MusicAnimationView setImages(int[] imageArray){
        ...
        if (mNotes != null) {
            for (Notes notes : mNotes) {
                if(notes!=null){
                    ...
                    int aa = (int) notes.X;
                    int bb = (int) notes.Y;
                    int cc= (int) (notes.X + mNotesIconHeight * notes.scale);
                    int dd =  (int) (notes.Y + mNotesIconHeight * notes.scale);

                    if(aa!=0&&bb!=0&&cc!=0&&dd!=0){
                        notes.mNotesIconDrawable.setBounds(aa, bb, cc, dd);
    ...
    private Drawable resize(Drawable image) {
        ...
        Bitmap bitmapResized = Bitmap.createScaledBitmap(b, 56, 56, false);
