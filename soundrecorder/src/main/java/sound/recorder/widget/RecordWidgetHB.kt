...
import sound.recorder.widget.databinding.LayoutEmptyHorizontalBlackBinding
...
class RecordWidgetHB : LinearLayout {
    ...
    private val imkasFragment = VoiceRecorderFragmentWidgetHorizontalBlack()
    private var isAdd = false
    private var binding: LayoutEmptyHorizontalBlackBinding


    constructor(_context: Context) : super(_context) {
        fragmentManagers = (_context as FragmentActivity).supportFragmentManager
        binding = LayoutEmptyHorizontalBlackBinding.inflate(LayoutInflater.from(context))
    }
    ...
    private fun setupViews(){
        fragmentManagers?.beginTransaction()?.replace(binding.recordWidgetHorizontalBlack.id, imkasFragment)?.commitAllowingStateLoss()
        if(!imkasFragment.isAdded){
            addView(binding.recordWidgetHorizontalBlack)
            isAdd = true
        }else{
            removeAllViews()
        }
